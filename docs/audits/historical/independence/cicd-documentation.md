...
The continuous integration pipeline is defined across 3 GitHub Actions workflow files inside the [.github/workflows/](.github/workflows) directory:

| Workflow File | Trigger Events | Purpose | Execution Environment | Status |
| :--- | :--- | :--- | :--- | :--- |
| [ci.yml](.github/workflows/ci.yml) | Push & Pull Requests to `main`, `master` | Compiles codebase, runs unit and architecture tests, audits NuGet vulnerabilities, runs static analysis, and builds Frontend/Mobile packages. | `ubuntu-latest` running .NET SDK `10.0.203` & Node `24`. | **PASS** |
| [tenant-isolation-certification.yml](.github/workflows/tenant-isolation-certification.yml) | Push & PRs to `main`, `develop`, Schedule (Nightly) | Runs SQL multi-tenant schema isolation, Row-Level Security, concurrency stress, and chaos engineering verification. | `ubuntu-latest` running Azure SQL Edge services container. | ⚠️ **FAIL (SDK Bug)** |
| [deploy-api.yml](.github/workflows/deploy-api.yml) | Push to `main`, Manual dispatch | Builds Docker container image and triggers deployment sequence to Dev, Staging, and Prod. | `ubuntu-latest` with Docker Buildx. | ⚠️ **FAIL (Mocked)** |
...
In the [tenant-isolation-certification.yml](.github/workflows/tenant-isolation-certification.yml) file, line 10 configures:
```yaml
env:
  DOTNET_VERSION: '8.0'
```
However, the codebase requires **.NET 10.0** (configured globally in [Directory.Build.props](Directory.Build.props#L11)). 
...
In [deploy-api.yml](.github/workflows/deploy-api.yml), the deployment steps to Azure (Azure Login, Docker push to Azure Container Registry, Bicep infrastructure deployment, Azure App Service container rollout) are commented out.
...
### Recommendations:
1. Update `DOTNET_VERSION: '10.0'` inside [tenant-isolation-certification.yml](.github/workflows/tenant-isolation-certification.yml).
2. Un-comment or configure the Azure parameters in [deploy-api.yml](.github/workflows/deploy-api.yml) with correct environmental secrets.
