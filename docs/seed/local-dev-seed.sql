...
--
--  2. Apply EF Core migrations for each bounded context against the LOCAL database.
--     From src/Backend/:
...
--
--     NOTE: Until the Provisioning service exists, create the tenant schema manually (step 3).
--
--  3. Create the dev tenant schema and apply RLS:
--       EXEC sp_executesql N'CREATE SCHEMA [tenant_dev]';
--       -- Then run docs/rls/bootstrap.sql and docs/rls/tenant_dev.sql (generated by RlsScriptGenerator)
--
--  4. Run this script:
--       sqlcmd -S "(localdb)\MSSQLLocalDB" -d Karamchari_Local -i docs\seed\local-dev-seed.sql
--
