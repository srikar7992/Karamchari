...
|---|---|---|---|---|---|
| C-1 | Critical | Single-DB schema-per-tenant scaling ceiling & SPOF | 6,12 | Platform/DB | **ARCHITECTURE APPROVED — IMPLEMENTATION PENDING** — [ADR 0018](docs/architecture/adrs/0018-tenant-storage-strategy.md) (Option C hybrid) **Accepted 2026-06-22** with placement authority + audit, online-migration runbook + cutover SLO, analytics-sourcing decision, per-tenant restore SLA, and dedicated→shared exit criteria. No longer an architecture risk; now a delivery program (phased migration). |
...
| H-2 | High | No saga/compensation for cross-module flows | 5 | Platform | **P1 IMPLEMENTED** — `EmployeeOnboardingSaga` waits for payroll provisioning + benefits enrollment, completes only when both done (2 harness tests green); Payroll now emits `PayrollProfileProvisionedIntegrationEventV1`. Follow-ups: durable EF saga persistence + scheduled timeout→`EmployeeOnboardingIncompleteIntegrationEventV1` escalation; sagas P2–P4 ([assessment](docs/architecture/SagaCandidateAssessment.md)) |
| H-4 | Med | Duplicate HTTP endpoint registrations → API host 500s on full-host requests | 8 | API host | **RESOLVED 2026-06-21** — removed duplicate `MapSuccessionEndpoints`/`MapCompensationEndpoints`/`MapAuditEndpoints` calls in `EndpointExtensions.cs` + duplicate `SuccessionModule`/`BenefitsModule` in `CapabilityRegistry`. 4 `RecruitmentApiCertificationTests` now green |
...
- **2026-06-22 (architecture review)** — **C-1 ARCHITECTURE APPROVED — IMPLEMENTATION PENDING.** [ADR 0018](docs/architecture/adrs/0018-tenant-storage-strategy.md) moved Proposed → **Accepted**. Five operationally-significant sections added before approval: (1) **Placement authority** — Placement Service = authority, `platform_directory` = source of truth, Rebalancer = enforcement; rules are data (`PlacementRules`); every decision writes an immutable `PlacementAudit` record. (2) **Online migration runbook** (shared→dedicated) — provision/seed/sync/cutover/validate/rollback, per-tenant write-freeze only, **cutover SLO < 60 s (max 5 min)**, `MigrationDryRunCompleted` gate before first prod migration. (3) **Analytics sourcing** — decided: operational DBs → outbox/CDC → event stream → warehouse; *cross-tenant reporting MUST NOT query operational DBs* (closes P2 reporting/OLAP item). (4) **Per-tenant restore SLA** — elevated to tier-backed product capability (ADR + recovery doc + commercial tiering + cert evidence). (5) **Placement exit criteria** — dedicated→shared explicitly **prohibited**; reversal requires a superseding ADR. C-1 is no longer an architecture risk; it is a phased delivery program. **Remaining work is operational:** C-2 (Azure deploy proof), C-3 (restore drill proof), H-3 (infra validation), C-1 (implementation). **Architecture work complete — shift to operational validation.**
